...
@section('content')
    ...
    <div class="banner text-center">
        <div class="container-fluid">
            <h2 class="text-white mb-20">Swap/Exchange <span class="segment-heading">    any of your valuable items online </span>
            </h2>
        ...
        <div class="container">
            ...
            <div class="product-desc">
                <div class="col-md-7 product-view">
                    <h2>{{ $item->name }}</h2>
                    <p><i class="glyphicon glyphicon-map-marker"></i> <a href="#">{{ $item->address }},</a> <a
                                href="#">{{ $item->state->name }}</a>| Added {{ $item->created_at->diffForHumans() }}
                    </p>
                    <div>
                        <div class="img-container text-center col-md-4 col-md-offset-4">
                            @if(count($item->images) > 0)
                                <img class="item-image-big" src="{{ $item->images[0]->url }}" alt="..."
                        ...
                        <div class="">
                            @if(count($item->images) > 0)
                                ...
                                @endforeach
                            @else
                                <img class="item-image-thumb" src="{{ asset('img/no-image-found.gif') }}"/>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="item-price">
                        ...
                        <div class="itemtype">
                            ...
                            <h5><a class="text-black"
                                   href="{{ url('/categories/'.$hashIds->encode($item->category->id).'/items') }}">{{ $item->category->name }}</a>
                            </h5>
                            <div class="clearfix"></div>
                        </div>
                    </div>
                    <div class="interested text-center">
                        <h4>Interested in this Ad?
